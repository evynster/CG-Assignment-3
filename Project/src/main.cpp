...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		GameObject nightEffectObject = scene->CreateEntity("NightVision Effect");
		{
			nightEffect = &nightEffectObject.emplace<NightVision>();
			nightEffect->Init(width, height);
		}
		effects.push_back(nightEffect);
		...
		std::vector<KeyPressWatcher> keyToggles;
		{
			...
			//toggle buffers
			keyToggles.emplace_back(GLFW_KEY_F1, [&]() { drawGBuffer ^= 1; });
			keyToggles.emplace_back(GLFW_KEY_F2, [&]() { drawIllumBuffer ^= 1; });

			keyToggles.emplace_back(GLFW_KEY_1, [&]() { 
				 gBuffer->light = true;
				 gBuffer->depth = false;
				 gBuffer->normal = false;
				 gBuffer->color = false;
				 gBuffer->accum = false;
				 drawGBuffer = false;
		...
		while (!glfwWindowShouldClose(BackendHandler::window)) {
			...
			if (drawGBuffer) {
				...
				effects[activeEffect]->ApplyEffect(illuminationBuffer);//this works
				greyscaleEffect->ApplyEffect(illuminationBuffer);
				...
				effects[activeEffect]->ApplyEffect(illuminationBuffer);
				nightEffect->myTime = glfwGetTime();
				nightEffect->ApplyEffect(illuminationBuffer);
				nightEffect->DrawToScreen();
			}
			else {
				effects[activeEffect]->ApplyEffect(illuminationBuffer);
				effects[activeEffect]->DrawToScreen();
